...
function handleRowBody(req, reqContentType='application/json'){
...
async function request({reqRow, routePath, reqParamsArr=[], reqContentType='application/json'}){

	const params = createParamsObj({reqParamsArr, reqRow, routePath});

	let rawBody = await handleRowBody(reqRow, reqContentType).catch((err)=>{ throw err; });
