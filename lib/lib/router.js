...
async function routerController({req, res, router, options}){
	...
	for(let route in routes){
		...
		if( reqRoutePath.match(routes[route].routeRegex) !== null ){
			if(req.method in routes[route].methods || req.method.toLowerCase() in routes[route].methods){
				...
				let appResponse = createAppResponse({res, options:{AccessControlAllowOrigin: options.AccessControlAllowOrigin} });
